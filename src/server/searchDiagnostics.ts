import type { SearchDiagnostics, SearchProviderDiagnostic } from "../shared/types.js";

export function emptySearchDiagnostics(): SearchDiagnostics {
  return {
    providers: [],
...
export function mergeSearchDiagnostics(...items: SearchDiagnostics[]): SearchDiagnostics {
  const providers = new Map<string, SearchProviderDiagnostic>();
  ...
  for (const item of items) {
    for (const diagnostic of item.providers) {
      ...
      if (diagnostic.skippedReason) current.skippedReason = diagnostic.skippedReason;
