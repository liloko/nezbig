...
export type StylometryAnalysis = {
  zipfScore: number;
  hapaxRatio: number;
  punctuationEntropyScore: number;
  paragraphUniformityScore: number;
  rhythmDeltaScore: number;
  evidence: string[];
};
 ...
 */
export function analyzeZipfAndVocabulary(tokens: string[]): { score: number; hapaxRatio: number; evidence: string[] } {
  if (tokens.length < 30) {
  ...
  if (totalTypes === 0) return { score: 0, hapaxRatio: 0.5, evidence: [] };

  let hapaxCount = 0;
  for (const count of frequency.values()) {
    if (count === 1) hapaxCount += 1;
  }
 ...
 */
export function analyzePunctuationEntropy(text: string, sentenceCount: number): { score: number; entropy: number; evidence: string[] } {
  if (sentenceCount < 4) {
 ...
 */
export function analyzeParagraphUniformity(paragraphs: string[]): { score: number; evidence: string[] } {
 ...
 */
export function analyzeSentencePacing(sentenceLengths: number[]): { score: number; meanDelta: number; evidence: string[] } {
 ...
 */
export function performStylometryAnalysis(text: string, proseSentences: string[], paragraphs: string[]): StylometryAnalysis {
