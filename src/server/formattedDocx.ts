...
function wordElements($: CheerioAPI, name: string, root?: Element): Element[] {
  const selector = name.replace(":", "\\:");
  const nodes = root ? $(root).find(selector).toArray() : $(selector).toArray();
  return nodes.filter((node): node is Element => node.type === "tag");
}
...
function replaceParagraphText($: CheerioAPI, paragraph: Element, revisedText: string): void {
  ...
  runs.forEach((run, index) => {
    ...
    $(run).text(value);
    if (/^\s|\s$/u.test(value)) $(run).attr("xml:space", "preserve");
    else $(run).removeAttr("xml:space");
  });
}

export async function mergeRevisedTextIntoDocx(source: Buffer, revisedText: string): Promise<Buffer> {
  const zip = await JSZip.loadAsync(source);
  const documentPart = zip.file(DOCUMENT_XML_PATH);
  if (!documentPart) throw new Error("DOCX не містить основної частини word/document.xml.");
