...
function alignTokensToParts(original: SourceToken[], revised: string[]): number[] {
  ...
  if (rows * columns > MAX_ALIGNMENT_CELLS) return proportionalPartMap(original, revised);
  ...
  for (let index = 0; index < mapped.length; index += 1) {
    if (mapped[index] >= 0) previousPart = mapped[index];
    else if (previousPart >= 0) mapped[index] = previousPart;
  }
  let nextPart = original.at(-1)?.partIndex ?? 0;
  for (let index = mapped.length - 1; index >= 0; index -= 1) {
    if (mapped[index] >= 0) nextPart = mapped[index];
    else mapped[index] = nextPart;
  }
  return mapped;
}

export function distributeRevisedText(originalParts: string[], revisedText: string): string[] {
...
export function alignParagraphs(original: string[], revised: string[]): string[] {
